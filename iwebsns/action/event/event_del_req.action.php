...
//定义写操作
  ...
  $sql="delete from $t_event_members where user_id=$member_id and event_id=$event_id";
  $dbo->exeUpdate($sql);
	$title=$ea_langpackage->ea_you_join.$event_name.$ea_langpackage->ea_activity_were_rejected;
	$scrip_content=$ea_langpackage->ea_you_join.'<a href="home.php?h='.$user_id.'&app=event_space&event_id='.$event_id.'" target="_blank">'.$event_name.'</a>'.$ea_langpackage->ea_activity_were_rejected;
